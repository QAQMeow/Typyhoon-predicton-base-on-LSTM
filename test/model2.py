...
model2.compile(loss='mse', optimizer =tf.keras.optimizers.Adam(learning_rate=0.0005, beta_1=0.9, beta_2=0.999))

history = model2.fit(train_X,train_Y,epochs=150,
...
modelname = 'model2 4steps '+str(Node)
#PrintLossFig(history,modelname+' Training and validation loss')

pre = model2.predict(test_X)
#pre = forecast(test_X,model2,4)
